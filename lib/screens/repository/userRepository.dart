...
Future <StudentModel> getUserDetails(String email)async{
  ...
  final userData = snapshot.docs.map((e) => StudentModel.fromSnapshot(e)).single;
  return userData;
}

Future <List<StudentModel>> allUser()async{
  final snapshot = await _db.collection("users").get();
  final userData = snapshot.docs.map((e) => StudentModel.fromSnapshot(e)).toList();
  return userData;
}
Future<void> updateUserRecord(StudentModel user)async{
